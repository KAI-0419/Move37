...
import { useGame, useMakeMove } from "@/hooks/use-game";
import { ChessBoard } from "@/components/ChessBoard";
import { Scanlines } from "@/components/Scanlines";
import { GlitchButton } from "@/components/GlitchButton";
import { TerminalText } from "@/components/TerminalText";
import { motion, AnimatePresence } from "framer-motion";
import { Loader2, AlertTriangle, Trophy, Radio, Terminal as TerminalIcon } from "lucide-react";
...
export default function GameRoom() {
  ...
  const handleSquareClick = async (r: number, c: number) => {
    ...
    // Helper to get piece at coord from string
    const getPiece = (r: number, c: number) => game.board[r * 5 + c];
    const clickedPiece = getPiece(r, c);
    const isMyPiece = clickedPiece !== '.' && clickedPiece === clickedPiece.toUpperCase();
    ...
    if (selectedSquare) {
      try {
        ...
        // Invalid move logic usually handled by backend, UI stays same, maybe flash error
            ...
            {game.winner ? (
               <motion.div
                 ...
                 className={cn(
                   ...
                   game.winner === 'player' ? "text-primary border-primary" : "text-destructive border-destructive"
                 )}
               >
                 {game.winner === 'player' ? "VICTORY ACHIEVED" : "SYSTEM DEFEAT"}
               </motion.div>
            ) : (
              <motion.div
                key={game.turn}
        ...
        {game.winner && (
           ...
           >
             <div className="text-center space-y-6 max-w-md p-8 border border-white/20 bg-black">
               ...
               <h2 className={cn("text-4xl font-display font-black", game.winner === 'player' ? "text-primary" : "text-destructive")}>
                 {game.winner === 'player' ? "YOU WON" : "YOU LOST"}
               </h2>
